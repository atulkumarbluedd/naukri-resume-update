...
class NaukriLoginClient:
    ...
    def login(self):
        response = self.session.post(
            LOGIN_URL, headers=self._get_headers(), json=self._get_payload()
        )
...
def update_resume() -> dict:
    ...
    profile_url = RESUME_UPDATE_URL_TEMPLATE.format(profile_id=profile_id)
    payload = {
        "textCV": {"formKey": form_key, "fileKey": FILE_KEY, "textCvContent": None}
    }
    ...
    try:
        send_telegram(bot_token, chat_id,
                    "✅ Naukri Resume Updated Successfully!\n\n"
                    f"📄 File: {final_filename}\n"
                    f"🕒 Updated At: {datetime.now().strftime('%d-%b-%Y %I:%M %p')}\n\n")
...
def handler(event, context):
    ...
    return {"status": update_resume(), "message": "Cron executed successfully"}
